...
public class PatientTimePickerF_b extends DialogFragment implements TimePickerDialog.OnTimeSetListener {
    ...
    public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
        ...
        Intent intent = new Intent(getContext(), PatientAlertReceiver_b.class);//동작안함
        PendingIntent operation = PendingIntent.getBroadcast(getContext(), 2, intent, 0);
        if(calendar.before((Calendar.getInstance()))){
            calendar.add(Calendar.DATE, 1);
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            //mAlarmManager.setExact(AlarmManager.RTC_WAKEUP, calendar.getTimeInMillis(), operation2);
    ...
    public void updateTimeText(Calendar c) {
        ...
        TextView mTextView = getActivity().findViewById(R.id.lunchtime);
